...
def _person_ids_from_nhs_no(session: Session, nhs_nos: Iterable[str]):
...
def _person_ids_from_mrn_no(session: Session, mrn_nos: Iterable[str]):
...
def _person_ids_from_ukrdc_no(session: Session, ukrdc_nos: Iterable[str]):
...
def _person_ids_from_full_name(session: Session, names: Iterable[str]):
...
def _person_ids_from_dob(session: Session, dobs: Iterable[Union[str, datetime.date]]):
...
def _person_ids_from_pidxref_no(session: Session, pid_nos: Iterable[str]):
...
def search_person(
    ...
    if nhs_number or search_list:
        match_sets.append(
            _person_ids_from_nhs_no(jtrace, (*nhs_number_list, *search_list))
        )

    if mrn_number or search_list:
        match_sets.append(
            _person_ids_from_mrn_no(jtrace, (*mrn_number_list, *search_list))
        )

    if ukrdc_number or search_list:
        match_sets.append(
            _person_ids_from_ukrdc_no(jtrace, (*ukrdc_number_list, *search_list))
        )

    if full_name or search_list:
        match_sets.append(
            _person_ids_from_full_name(jtrace, (*full_name_list, *search_list))
        )

    if dob or search_list:
        match_sets.append(_person_ids_from_dob(jtrace, (*dob_list, *search_list)))

    if pidx or search_list:
        match_sets.append(
            _person_ids_from_pidxref_no(jtrace, (*pidx_list, *search_list))
        )
