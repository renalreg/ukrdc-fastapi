...
def test_persons_list(client):
    response = client.get("/empi/persons")
...
def test_persons_list_clpid_filter_single(client):
    response = client.get("/empi/persons?clpid=123456789")
    assert response.status_code == 200
    returned_ids = {item["id"] for item in response.json()["items"]}
...
def test_persons_list_ukrdcid_filter_multiple(client):
    response = client.get("/empi/persons?clpid=123456789&clpid=987654321")
    assert response.status_code == 200
    returned_ids = {item["id"] for item in response.json()["items"]}
...
def test_person_detail(client):
    response = client.get("/empi/persons/1")
    assert response.status_code == 200
...
def test_person_detail_not_found(client):
    response = client.get("/empi/persons/9999")
...
def test_person_masterrecords(client, jtrace_session):
    ...
    response = client.get("/empi/persons/1/masterrecords")
    assert response.status_code == 200
