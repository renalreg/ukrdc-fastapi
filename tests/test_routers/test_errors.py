...
from ukrdc_fastapi.routers.api.errors.messages import ExtendedErrorSchema
...
def test_errors_list(client):
    ...
    response = client.get(f"/api/errors/messages/?since={since}&until={until}")
...
def test_errors_list_facility(client):
    ...
    until = datetime.datetime(2021, 12, 12, 23, 59, 59).isoformat()
    response = client.get(
        f"/api/errors/messages/?since={since}&until={until}&facility=TEST_SENDING_FACILITY_2"
    )
    assert response.status_code == 200
    messages = [MessageSchema(**item) for item in response.json()["items"]]
    returned_ids = {item.id for item in messages}
...
def test_errors_detail(client):
    response = client.get("/api/errors/messages/1")
